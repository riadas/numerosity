...
function generate_languages(;all_languages=false)
    ...
    global L0_spec = specs[3] # non-knower
    global L1_spec = specs[4] # one-knower
    global L2_spec = specs[5] # two-knower
    global L3_spec = specs[6] # three-knower

    # two-knower approx
    global L25_spec = deepcopy(L2_spec)
    L25_spec["approx"] = true

    # three-knower approx
    global L35_spec = deepcopy(L3_spec)
    L35_spec["approx"] = true

    # four-knower
    global L4_spec = deepcopy(L3_spec)
    L4_spec["four_definition"] = "set.value == 4"

    # CP-unit-knower
    global L7_spec = specs[end]
    L7_spec["parallel_individuation_limit"] = 4
    L7_spec["ANS_reconciled"] = true
    L7_spec["unit_add"] = "unit_add_final"

    # CP-mapper
    global L6_spec = deepcopy(L7_spec)
    L6_spec["ANS_reconciled"] = true
    L6_spec["unit_add"] = "unit_add_base"

    # CP-knower
    global L5_spec = deepcopy(L6_spec)
    ...
    (language_names, language_names_pretty, language_name_to_spec, tups)    
end

# language_names, language_names_pretty, language_name_to_spec = generate_languages(all_languages=false)
...
english_dataset = Dict([
    "give_1" => 80.0 - 0.5, # = GiveN("one") 40 / 80
    "give_2" => 20.0 - 0.5, # = GiveN("two") 20 / 40
    "give_3" => 9.0 - 0.5, # = GiveN("three")
    "give_4" => 5.0 - 0.5, # = GiveN("four")
    "give_5" => 3.0 - 0.5, # = GiveN("five")
    "give_6" => 2.0 - 0.5, # = GiveN("six")
    "give_7" => 2.0 - 0.5, # = GiveN("seven")
    "give_8" => 1.0 - 0.5, # = GiveN("eight")
    "give_9" => 1.0 - 0.5, # = GiveN("nine")
    "give_10" => 1.0 - 0.5, # = GiveN("ten")

    "how_many_1" => 0.5, #  = HowMany(Exact(1))
    "how_many_2" => 0.5, #  = HowMany(Exact(2))
    "how_many_3" => 0.5, #  = HowMany(Exact(3))
    "how_many_4" => 0.5, #  = HowMany(Exact(4))
    "how_many_5" => 0.5, #  = HowMany(Exact(5))
    "how_many_6" => 0.5, #  = HowMany(Exact(6))
    "how_many_7" => 0.5, #  = HowMany(Exact(7))
    "how_many_8" => 0.5, #  = HowMany(Exact(8))
    "how_many_9" => 0.5, #  = HowMany(Exact(9))
    "how_many_10" => 0.5, #  = HowMany(Exact(10))
...
function compute_base_accuracies(dataset, full_reset=true)
    ...
    for language_name in language_names
        ...
        if occursin("LXP", language_name)
            ...
            defined_subset_indices = findall(x -> x != 5, tup)
            defined_subset_numbers = map(i -> tup[i], defined_subset_indices)
            undefined_subset_indices = findall(x -> x == 5, tup)
...
function run_test(test_name_, normalized=true, intervention=false, intervention_small=false, intervention_count=false, save_fig_title="")
    ...
    accuracies_plot = accuracies[1:length(language_names_pretty_bar_plot)]
    memory_costs_plot = accuracies[1:length(language_names_pretty_bar_plot)]
    computation_costs_plot = accuracies[1:length(language_names_pretty_bar_plot)]

    accuracy_plot = bar(map(x -> join(split(x, "_")[2:end], " "), language_names_pretty_bar_plot), accuracies_plot, color = modified_colors, xrotation=305, size=(600, 525), legend=false, xlabel="LoT Stage", ylabel="Accuracy", title="Task Accuracy", ylims=(0.0, 1.0))

    memory_cost_plot = bar(map(x -> join(split(x, "_")[2:end], " "), language_names_pretty_bar_plot), memory_costs_plot ./ maximum(memory_costs_plot), color = modified_colors, xrotation=305, size=(600, 525), legend=false, xlabel="LoT Stage", ylabel="Cost", title="Memory Cost", ylims=(0.0, 1.0))

    computation_cost_plot = bar(map(x -> join(split(x, "_")[2:end], " "), language_names_pretty_bar_plot), computation_costs_plot ./ maximum(computation_costs_plot), color = modified_colors, xrotation=305, size=(600, 525), legend=false, xlabel="LoT Stage", ylabel="Cost", title="Computation Cost", ylims=(0.0, 1.0))
    ...
    for i in 1:length(language_names)
        ...
        if isnothing(line_plot)
            line_plot = plot(x_vals, y_vals, size=(600, 450), xlims=(0.0, x_vals[end]), ylims=(min_yvals, max_yvals), legend=:bottomright, label=occursin("LXP", language_names_pretty[i]) ? "" : join(split(language_names_pretty[i], "_")[2:end], " "), color = modified_colors[i], title="Utility vs. Cost Tolerance (Time)", xlabel="Cost Tolerance (Time)", ylabel="Utility")
        else
            line_plot = plot(line_plot, x_vals, y_vals, size=(600, 450),  xlims=(0.0, x_vals[end]), ylims=(min_yvals, max_yvals), legend=:bottomright, label=occursin("LXP", language_names_pretty[i]) ? "" : join(split(language_names_pretty[i], "_")[2:end], " "), color = modified_colors[i], title="Utility vs. Cost Tolerance (Time)",  xlabel="Cost Tolerance (Time)", ylabel="Utility")
        end
        yvals_dict[i] = y_vals
    end
    ...
    for t in 0:time_step_unit:num_time_steps*time_step_unit
        ...
        if intervention && three_knower_stage_reached && !three_knower_stage_intervention_made
            ...
            global accuracies = compute_accuracies_efficient(task_dict, normalized, false, intervention)
